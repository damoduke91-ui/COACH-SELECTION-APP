-- Automatically submit the latest previously submitted team for any coach who
-- is still unsubmitted when the scheduled lockout becomes active.
--
-- The function is deliberately idempotent:
--   * a transaction-scoped advisory lock serialises concurrent invocations;
--   * already-submitted current-round snapshots are never overwritten; and
--   * the existing coach/round unique index is used for the snapshot upsert.
...
BEGIN
  ...
  -- Prevent two scheduler requests from processing the same environment at once.
  ...
  -- A saved lockout remains in the past until the next schedule is saved. Do
  -- not let that stale timestamp submit teams if an admin advances the round
  -- first. The scheduler runs every minute, so 24 hours is a generous retry
  -- window while still keeping the action tied to the intended lockout.
  ...
  LOOP
    ...
    WHERE previous_submission.environment = requested_environment
      ...
      AND previous_submission.round_number < settings_row.current_super8_round
      AND previous_submission.is_submitted = true
    ORDER BY
      previous_submission.round_number DESC,
    ...
    IF NOT FOUND THEN
      ...
      skipped_coaches := skipped_coaches || jsonb_build_array(
        jsonb_build_object(
          ...
          'reason', 'no_previous_submitted_team'
        )
      );
      CONTINUE;
    END IF;
    ...
    submitted_coaches := submitted_coaches || jsonb_build_array(
      jsonb_build_object(
        'coach_id', selection_row.coach_id,
        'coach_name', selection_row.coach_name,
        'source_round', previous_row.round_number
      )
    );
  END LOOP;
